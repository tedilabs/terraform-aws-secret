...
variable "name" {
  description = "(Required) Name of the replica key."
  type        = string
  nullable    = false
}
...
variable "description" {
  description = "(Optional) The description of the replica key."
  type        = string
...
variable "enabled" {
  description = "(Optional) Whether the replica key is enabled. Disabled replica keys cannot be used in cryptographic operations. Defaults to `true`."
  type        = bool
  default     = true
  nullable    = false
}
...
variable "grants" {
  description = <<EOF
  (Optional) A list of grants configuration for granting access to the replica key. Each item of `grants` as defined below.
...
variable "predefined_policies" {
  description = <<EOF
  (Optional) A configuration for predefined policies of the replica key. This configuration will be merged with given `policy` if it is defined. Each item of `predefined_policies` block as defined below.
    (Required) `role` - The predefined role to be applied to the replica key. Valid values are `OWNER`, `ADMINISTRATOR`, `USER`, `SERVICE_USER`, `SYMMETRIC_ENCRYPTION`, `ASYMMETRIC_ENCRYPTION`, `ASYMMETRIC_SIGNING`, or `HMAC`.
      `OWNER` - Full access to the replica key, including permission to modify the key policy and delete the key.
      `ADMINISTRATOR` - Administrative access to the replica key, including permission to modify the key policy, but not permission to delete the key.
      `USER` - Access to use the replica key for cryptographic operations, but not administrative permissions.
      `SERVICE_USER` - Access for AWS services to use the replica key for cryptographic operations on your behalf.
      `SYMMETRIC_ENCRYPTION` - Access to use the replica key for symmetric encryption and decryption operations.
      `ASYMMETRIC_ENCRYPTION` - Access to use the replica key for asymmetric encryption and decryption operations.
      `ASYMMETRIC_SIGNING` - Access to use the replica key for asymmetric signing and verification operations.
      `HMAC` - Access to use the replica key for HMAC generation and verification operations.
    (Required) `iam_entities` - A set of ARNs of AWS IAM entities who can be permitted to access the replica key for the predefined role.
    (Optional) `conditions` - A list of required conditions to be met to allow the predefined role access to the replica key. Each item of `conditions` block as defined below.
